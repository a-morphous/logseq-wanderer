...
const App: React.FC = () => {
	const [parser, _setParser] = useState(new Parser())
	const [noteName, setNoteName] = useState(() => {
		logseq.Editor.getCurrentPage()
			...
			.catch((e) => {
				console.log("Did not manage to get current page for note name", e)
			})
		return "noteName"
	})
	...
	const [mappedCategoryValues, setMappedCategoryValues] = useState([{ category: "category" }])

	//create a function to handle change of inputs
	const handletagsChange = (event: React.ChangeEvent<HTMLInputElement>, index) => {
	...
	const handleCategoryChange = (event: React.ChangeEvent<HTMLInputElement>, index) => {
		const { name, value } = event.target
			...
			<div>
				<div className="flex justify-center">
					<div className="smartblock-inserter">
						<h1 className="full-width text-left text-2xl font-bold">Hugo Export</h1>
						<br></br>
						...
						<div className="flex justify-center">
							<button
								...
								onClick={async () => {
									parser.generateZip(await logseq.Editor.getCurrentBlock(), {
										tags: mappedtagsValues,
										date: {
											updatedDate,
											originalDate
										},
